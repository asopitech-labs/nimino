...
`window.clearWebViewProfileData({webViewCookies, webViewLocalStorage, webViewCache})`はWebView engineが所有するCookie・localStorage・cacheを対象にする明示APIです。Windows WebView2ではCookie単独を`ICoreWebView2CookieManager::DeleteAllCookies`、複数種別を`ICoreWebView2Profile2::ClearBrowsingData`へ接続し、完了callbackをFutureへ伝播します。Linux WebKitGTKでは、Windowに接続済みの`WebKitNetworkSession`から同一の`WebKitWebsiteDataManager`を取得し、公式の[`clear()`](https://webkitgtk.org/reference/webkit2gtk/2.42.4/method.WebsiteDataManager.clear.html)を`timespan = 0`で呼び出します。Cookieは`COOKIES`、localStorageは`LOCAL_STORAGE`、cacheは`MEMORY_CACHE`・`DISK_CACHE`・`OFFLINE_APPLICATION_CACHE`・`DOM_CACHE`を消去し、完了callbackをFutureへ伝播します。未準備/終了済みWebViewは`invalidState`です。WSL host relayはLinux APIをリンクせず、Windows host protocolにもこの非同期ライフサイクルがないため、明示的に`platformUnavailable`を返します。実Windows Evergreen Runtimeでのcallback実行はWindows GUI環境で別途検証します。
