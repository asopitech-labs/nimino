...
**状態: M4部分実装。Windows/Linux向けの`App`/`Window` facade、同一Windowの複数`WebView`管理、Window単位の明示許可リストJSON RPC、WebView bootstrap、profile path、local asset root/entry境界、native menu、system tray、native notificationを実装した。Linuxのsystem trayはsession D-BusにStatusNotifierWatcherがある場合だけCapabilityを有効にし、ない環境では`unsupported`を返す。`registerTyped` / `registerTypedAsync`は標準JSON codecで型付きhandlerを登録でき、`typescriptDeclarations`で複合型を含む宣言を生成できます。Linuxでは実WebViewのrequest/response/notification往復、URLの最初期scriptからのRPC、WebKitWebsiteDataManagerによるbrowser data clearを確認済みで、Windowsはx64クロスコンパイル済みです。WSL build（`-d:niminoWsl`）は同じcore APIから認証済みWindows hostを選び、メニュー・トレイ・通知・ファイルダイアログをIPC中継します。Windows WebView2 Runtime上でWSL coreの読込、評価、async response、timeout、URL document-start RPCを確認済みです。permission/download/navigationはnative実装とWSL同期decision relay（timeout時deny）まで完了し、profile設定/Cookieの永続化API、Window単位の参照・削除APIも実装済みです。`onNotificationActivated`はWindows WinRT Toastまたは通知領域balloonのクリックをCore/WSLへ中継し、終了済みプロセスはToastActivatorCLSIDのCOM local-serverを介して同じcallbackへ復帰します。WebView内部custom protocolはWindows/Linux nativeとWSL認証済み同期relayまで実装済みです。実Windows WebView2 Runtime上のWSL往復はGUI実機ハーネスで別途検証します。署名済み配布、正式なMSI更新の実機検証、macOSは後続ターゲットです。**
...
`window.clearWebViewProfileData({webViewCookies, webViewLocalStorage, webViewCache})`はWebView engineが所有するCookie・localStorage・cacheを対象にする明示APIです。Windows WebView2ではCookie単独を`ICoreWebView2CookieManager::DeleteAllCookies`、複数種別を`ICoreWebView2Profile2::ClearBrowsingData`へ接続し、完了callbackをFutureへ伝播します。Linux WebKitGTKでは、Windowに接続済みの`WebKitNetworkSession`から同一の`WebKitWebsiteDataManager`を取得し、公式の[`clear()`](https://webkitgtk.org/reference/webkit2gtk/2.42.4/method.WebsiteDataManager.clear.html)を`timespan = 0`で呼び出します。Cookieは`COOKIES`、localStorageは`LOCAL_STORAGE`、cacheは`MEMORY_CACHE`・`DISK_CACHE`・`OFFLINE_APPLICATION_CACHE`・`DOM_CACHE`を消去し、完了callbackをFutureへ伝播します。未準備/終了済みWebViewは`invalidState`です。WSLでは、認証済みready capabilityに`webViewProfileDataClear`を含むWindows hostだけが、実行中のUI sessionでこの非同期requestを`native.webview.clearBrowsingData`として中継します。coreはrequest IDごとにFutureを保持し、hostからの成功または構造化native errorを同じFutureへ伝播します。古いhostなどこのcapabilityを持たない接続では`platformUnavailable`、UI開始前・終了後は`invalidState`です。relay capabilityはtransportの対応だけを示すため、WebView2 runtimeが特定種別を未対応として返した場合も、その`platformUnavailable`を成功へ変換しません。実Windows Evergreen Runtimeでのcallback実行はWindows GUI環境で別途検証します。
WSL hostの低水準`native.window.clearCache`は、active user-data folderの直接削除を避けるため明示的に未対応です。WebView engine内部データを消去する場合は、対応するProfile APIを通す必要があります。
...
`CoreWindowOptions.zoomFactor`はWebViewの初期倍率（0.25〜5.0、既定1.0）です。Windowsは
WebView2 Controller、LinuxはWebKitGTKの`webkit_web_view_set_zoom_level`へ設定し、WSLでは
Windows hostへ中継します。範囲外は`invalidArgument`になります。
