...
通知します。生成hostでは同じ配列を`nimino-file-drop` CustomEventの`detail`としてページへ通知します。
未指定時はWebViewの標準ドロップ処理を維持します。
macOS generated hostの`permissions.allow`は`camera`/`microphone`だけを受け付けます。
`notifications`、`geolocation`、`clipboard`、`screenCapture`は起動前に拒否され、
`runtime.systemTrayIcon`はmacOS packageのResourcesへ同梱したローカル画像だけを受け付け、
生成hostのNSStatusItemへ設定されます。未指定時はアプリケーションiconをtrayの既定iconとして使います。
`webview.wasm`はmacOS WKWebViewへ伝播します。WKWebViewではWebAssembly自体は標準で有効ですが、
Chromium専用browser argsは利用できないため、SharedArrayBufferには配信元のCOOP/COEPが必要です。
...
これらは`--host`に渡した実行ファイルと同じディレクトリから取得します。packは、hostの隣にあり、かつhostが名前で参照しているライブラリだけをbundleへ配置します。OS標準のDLLはhostの隣に存在しないため対象になりません。参照されないファイルを同じディレクトリへ置いても同梱しません。リリース資産の`nimino-core-<version>-windows-x64.zip`はこの配置で公開しているため、展開して`--host`をその中の`nimino-host.exe`へ向ければ、利用者がライブラリを個別に指定する必要はありません。
