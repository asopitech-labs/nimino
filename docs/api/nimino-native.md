...
GTK4/GLibには、従来のstatus iconを常駐させる正式なsystem-tray APIがありません。そのため
Linuxの`systemTray` Capabilityは`false`で、`configureSystemTray`は
`unsupported`（`GTK4/GLib provide no supported system-tray or status-icon API`）を返します。
tray依存の導線は`configureNativeMenu`でアプリメニューを提供し、状態変化は
`sendNativeNotification`で通知する構成に置き換えてください。外部のAppIndicator実装を
暗黙に導入してtray対応済みとは扱いません。
...
通知領域balloonのクリックを`onNotificationActivated`で受け取れます。ToastはAppUserModelIdを
要求し、各Toastのlaunch IDをcallbackへ伝えます。Linux/WSLの`onNotificationActivated`登録は
`unsupported`を返します。
