...
macOSでは`app.setDockBadge("3")`でDock tileへバッジを設定し、空文字列で消去できます。生成hostはWeb標準の`navigator.setAppBadge()` / `navigator.clearAppBadge()`と`window.nimino.setDockBadge()`へ同じ操作を公開します。
...
macOSはCocoa/WebKitのprivate bridgeで`NSApplication`、`NSWindow`、複数`WKWebView`、階層native menu、`NSStatusItem` tray、`NSDockTile` badge、`UNUserNotificationCenter`、通知activation、`NSApplication`の`openURLs` deep linkを提供します。通知登録時はAlert/Sound/Badge権限を要求し、activationは`UNNotificationRequest.identifier`で通知します。Apple-issuedなTeam IDを持たない未署名・Ad-hoc bundleでは通知要求を事前に拒否し、生成hostはin-app bannerへフォールバックします。`onPermissionRequested`はWKWebKitのmedia capture（camera/microphone）をdeny-defaultで判定し、`onDownloadStarting`/`onDownloadPath`/`onDownloadEvent`はWKDownloadの開始・ユーザーDownloads保存先・進捗・完了/失敗へ接続します。Dockクリック時の`applicationShouldHandleReopen`では、非表示の全Windowを再表示します。

macOSの未署名／Ad-hocローカルbundleは起動・WebView・Deep Link等の確認には使えますが、現行macOSのUserNotificationsサービスに拒否される場合があるため、通知表示・クリックはApple-issuedな開発用署名で確認してください。通知要求が拒否された場合、bridgeはstderrへ開発署名またはin-app fallbackの案内を出します。
