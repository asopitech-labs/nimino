...
- protocol versionは互換範囲ではなくwire contractの完全一致で判定する。現行はv2であり、hostは`hello.version`、clientは`ready.version`をnative object生成・session利用より前に検証する。各受信側validatorは不一致を`unsupportedVersion`として拒否し、v1 host/clientとのfallbackやdowngradeは行わない。認証前の不一致にはhostがprotocol responseを返さず終了するため、旧clientが受けるtransport errorを`unsupportedVersion`と保証するものではない。配布・更新時はWSL clientと`nimino-wsl-host.exe`を同じprotocol versionの組で更新する。
- v2の認証済み`ready.payload`は、既知のnative Capability名だけから成る重複なしの`capabilities`配列である。clientはこのsnapshotの検証に成功するまでsessionを採用しない。Capability追加は、protocol側の許可名更新とprotocol version更新を同時に行う。
