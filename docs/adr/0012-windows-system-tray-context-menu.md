...
hide-on-close、start-to-tray、アプリ固有アイコンは `nimino-core` と
...
- icon は Windows 標準の `IDI_APPLICATION` を使う。アプリ icon 供給は pack の
  配布設定と統合するまで追加しない。
- Windows toast notification は `nimino-native` のWinRT実装で提供する。
  `nimino-pack` は AppUserModelID を Start-menu shortcut の
  `System.AppUserModel.ID` propertyへ設定し、per-user installer registryを生成する。
  起動中プロセスは `ToastNotification.Activated` callback、終了済みプロセスは
  launcherが転送する `launch` command-line payloadを同じ
  `onNotificationActivated` APIへ届ける。Toastの表示・activationはWindows GUI実機で
  検証するため、Dockerのcross compileだけでは表示成功を主張しない。
