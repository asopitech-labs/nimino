...
- OS deep linkはWebView custom protocolとは別のpackaging契約として実装する。`nimino-pack`の`[deepLink] schemes`を入力に、Windows per-user registry（PowerShell/NSIS/MSI）とLinux Desktop Entryの`x-scheme-handler/*`を生成する。起動URIはlauncher引数としてhostへ転送する。実行中アプリへの再利用・WSL host activation relayは別のruntime API契約であり、このpackaging契約では暗黙に追加しない。
