...
Accepted — M6の正式packager前段として、platform metadataとWindows per-user導入templateを生成する。
...
- 正式なMSI/NSIS、code signing、`deb`/RPM/AppImage/Flatpakはこの段階で生成しない。platform packagerはmetadataを入力に実装し、対象OSで実機検証・署名を行う。
...
- Linux desktop entryはmetadataとして検査する。実際の`/opt`配置、icon theme integration、desktop database更新、deb/RPM/AppImage生成は別packagerの責務である。
