...
Accepted — M6の正式packager前段としてplatform metadataを生成し、LinuxではDebian/RPM archiveとFlatpak contextを生成する。AppImageはADR-0019のfail-closed境界、Windowsの署名済みinstallerは後続とする。
...
- Debian/RPM archiveは`package-linux`で生成・内容検証する。AppImageは固定依存を検査するが、依存ライブラリ同梱、署名、更新情報が未実装のため成果物生成を拒否する。
- MSIとcode signingはこの段階で生成しない。Flatpakは固定GNOME runtimeを参照するbuild context manifestとbundle sourceを生成するが、flatpak-builderによる実bundle生成・署名・実機検証は後続とする。Windows NSISは別ADRのDocker生成経路で実装済みだが、対象OSでの実機検証・署名は後続とする。Windowsのper-user PowerShell templateも引き続き提供する。
...
- Linux desktop entryはmetadataとして検査し、Debian/RPM archiveにも同じlayoutを含める。AppImageへ含めるlayoutはdependency closureと一体で再検証する。実際の`/opt`配置、icon theme integration、desktop database更新、署名は別packagerの責務である。
