...
Accepted — M6の正式packager前段としてplatform metadataを生成し、LinuxではDebian/RPM archiveとFlatpak contextを生成する。Flatpak実bundleとAppImage閉包はDocker専用packaging smokeで検証し、署名はrelease gateとする。
...
- Debian/RPM archiveは`package-linux`で生成・内容検証する。AppImageは固定依存を閉包して生成するが、署名、更新情報、配布先起動検証はrelease gateとする。
- MSIはDocker内のWiX互換`wixl`で生成する。Flatpakは固定GNOME 49 runtimeを参照するmanifestから専用Docker serviceで実bundleをexportする。Windows NSIS/MSIは対象OSでの実機検証・署名を後続とし、Windowsのper-user PowerShell templateも引き続き提供する。
