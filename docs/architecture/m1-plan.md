...
M2の縦機能として、native の `webkit_web_view_evaluate_javascript`/`_finish` と WebView2 `ExecuteScript`、文字列message、ナビゲーション完了を実装した。Linuxのナビゲーション完了は`WebKitWebView::load-changed`と`::load-failed`、WindowsはWebView2 `NavigationCompleted`を使用し、WSL hostはeventとして中継する。残る M2 API は`WebKitWebView::decide-policy`、`::create`、navigation開始、new-window、error eventである。HTML読込にはM1から`webkit_web_view_load_html`を使用する。GTKのレイアウトがWindowリサイズへ追従するため、WindowsのようなBounds更新は不要である。
