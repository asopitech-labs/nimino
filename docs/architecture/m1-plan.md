...
M2の縦機能として、native の `webkit_web_view_evaluate_javascript`/`_finish` と WebView2 `ExecuteScript`、文字列message、ナビゲーション開始/完了を実装した。Linuxの開始は`WebKitWebView::decide-policy`、完了は`::load-changed`と`::load-failed`、WindowsはWebView2 `NavigationStarting`/`NavigationCompleted`を使用し、WSL hostはeventとして中継する。Windows/Linuxは開始callbackによる中止を実装する一方、WSL client同期判定は[ADR-0005提案](../adr/0005-wsl-navigation-policy.md)のスパイク待ちである。残る M2 API は`::create`、new-window、error eventである。HTML読込にはM1から`webkit_web_view_load_html`を使用する。GTKのレイアウトがWindowリサイズへ追従するため、WindowsのようなBounds更新は不要である。
