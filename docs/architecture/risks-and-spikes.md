...
| R14 | RPC Futureの完了/timeout回収をUI loopへ安全に統合できない | timeoutが発火しない、またはUIを塞ぐ | Linux GLib sourceからの`tick()`で実WebViewの同期/非同期/timeoutを確認済み。Windows timerはクロスコンパイル済み。WSL core relayはfake hostで確認済み。Windows RuntimeとWSL実WebView2 async/timeout経路は未確認。Nim asyncdispatchの独自loopは作らない |
