...
wsl-host-smoke: image ## WSLからWindows hostのWebView2生成・HTML・JavaScript・shutdownを実機確認する
	...
	(timeout --foreground 120s powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$$(wslpath -w $(CURDIR)/tools/ci/wsl-host-smoke.ps1)" -HostExecutable "$$(wslpath -w $(CURDIR)/.tmp/nimino-wsl-host.exe)") || { status=$$?; taskkill.exe /IM nimino-wsl-host.exe /F >/dev/null 2>&1 || true; exit $$status; }

wsl-host-abnormal-smoke: image ## WSL clientのstdin異常終了時にWindows hostが終了することを確認する

	$(COMPOSE) run --rm $(SERVICE) nimble buildWslHostArtifact
	(timeout --foreground 120s powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$$(wslpath -w $(CURDIR)/tools/ci/wsl-host-smoke.ps1)" -HostExecutable "$$(wslpath -w $(CURDIR)/.tmp/nimino-wsl-host.exe)" -AbnormalClientEof) || { status=$$?; taskkill.exe /IM nimino-wsl-host.exe /F >/dev/null 2>&1 || true; exit $$status; }
...
wsl-host-popup-smoke: image ## WebView2新規Window要求・明示popup message受信を実機確認する
	...
	$(COMPOSE) run --rm $(SERVICE) nimble buildWslHostArtifact
	(timeout --foreground 120s powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$$(wslpath -w $(CURDIR)/tools/ci/wsl-host-smoke.ps1)" -HostExecutable "$$(wslpath -w $(CURDIR)/.tmp/nimino-wsl-host.exe)" -VerifyNewWindow) || { status=$$?; taskkill.exe /IM nimino-wsl-host.exe /F >/dev/null 2>&1 || true; exit $$status; }
