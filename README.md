...
URLのRPC bridgeはViewが`pending`の間の最初の対象`loadUrl`前に登録し、HTTP(S)は初回URLと同一origin、`data:`は完全一致のURLだけで初期化します。`about:`を含むほかのschemeと後続の別originではRPCを公開しません。次はWSL経路へのpermission/download decision relay、WSL client側navigation判定、デスクトップ統合、型抽出です。
