...
```bash
docker run --rm -v "$PWD:/work" ghcr.io/asopitech-labs/nimino:latest \
  pack https://example.com --out /work/dist/app
```

It runs as uid 1000, so files it writes into the mounted directory belong to
the host user rather than to root.
...
**Windows installer behavior:** NSIS and MSI installers check the WebView2 Evergreen Runtime and download the official Microsoft Bootstrapper only when the runtime is missing. Internet access is required for that first-time download. `WebView2Loader.dll` and `pcre64.dll` are bundled with the application: `pack` stages the libraries that sit beside the `--host` executable and are named by it, so unpacking `nimino-core-<version>-windows-x64.zip` and pointing `--host` at the `nimino-host.exe` inside it is enough.
