...
> M0〜M4のprofile、local asset境界、navigation/permission/download policy、Windows tray/WinRT Toast notification、Linux GTK menubar/notification、Windows/GTK/WSLのOSファイルダイアログを実装済みです。`nimino-pack`はLinux desktop entry（`[deepLink] schemes`の`x-scheme-handler/*`を含む）、Debian/RPM archive、Flatpak build context、Docker内のGNOME 49実bundle、Windowsのper-user NSIS/MSI setup（URL Protocol/Toast COM LocalServer registryを含む）、CycloneDX SBOM、PowerShell templateを生成します。AppImageは固定依存preflight、GTK/WebKitGTK dependency closure、補助process再配置、AppDir生成まで実装し、不完全な閉包はfail-closedで拒否します。署名、clean target runtimeでのGUI起動、通常Windows GUI CI、macOSは未整備です。NSIS/MSI setupのWindows実機install/upgradeとToastのShell起動は未検証です。
...
主なターゲットは`make image`（image作成）、`make verify-env`（Nim/Nimble/GTK/WebKitGTK検証）、`make shell`（コンテナshell）、`make test`（単体テストとfake hostによるWSL core RPC）、`make pack-prepack-test`（組み込みprepack定義テスト）、`make pack-prepacks-test`（YouTube/Gmail/Google Analytics bundle生成テスト）、`make linux-smoke`（native Linux GUI smoke）、`make core-linux-rpc-smoke`（core RPC同期往復）、`make core-linux-rpc-url-smoke`（core URLのdocument-start RPC実往復）、`make core-linux-rpc-async-smoke`（core RPC async/timeout実往復）、`make windows-cross`（Windows native x64クロスコンパイル）、`make core-windows-cross`（Windows core x64クロスコンパイル）、`make wsl-host-cross`（Windows WSL hostクロスコンパイル）、`make wsl-host-smoke`（host単体のWebView2実機確認）、`make wsl-prepack-smoke`（YouTube/Gmail/Google Analytics実サイト読込確認）、`make wsl-host-abnormal-smoke`（client EOF時のhost終了確認）、`make wsl-host-popup-smoke`（WebView2新規Window要求と暗黙popup抑止の実機確認）、`make wsl-client-smoke`（通常WSL client APIでWindows hostを起動する実機確認）、`make wsl-core-smoke`（通常core APIでWindows hostを選ぶ実機確認）、`make wsl-core-rpc-url-smoke`（Windows WebView2上のWSL core URL document-start RPC実機確認）、`make wsl-core-rpc-async-smoke`（Windows WebView2上のWSL core async RPC/timeout実機確認）、`make clean`（Compose資源と一時成果物の削除）です。
