...
URLのRPC bridgeはViewが`pending`の間の最初の対象`loadUrl`前に登録し、HTTP(S)は初回URLと同一origin、`data:`は完全一致のURLだけで初期化します。`about:`を含むほかのschemeと後続の別originではRPCを公開しません。WSL経路のpermission/download/navigation relay、型付きRPC、profile/Cookie同期、外部ナビゲーション、ローカル・明示許可リモートasset処理まで実装済みです。残る大項目はWebView engine Cookie/cache managerの完全接続、OSデスクトップ統合、複合型の自動TypeScript抽出です。
