...
M0の責務境界、公開API案、所有権、イベントループ、WSL IPC、Capability、技術リスク、M1計画は文書化済みです。M1のWindow/WebView/URL/HTML/タイトル/終了、M2のJavaScript評価、文字列message、ナビゲーション完了を、Windows・Linux・WSLの共通API形状で実装しています。

- Linux: `make linux-smoke` が URL/HTML、JavaScript評価、文字列message、ナビゲーション完了、明示解放を実行します。
...
次はM2のnavigation開始/new-window/error event、続いてM3の`nimino-core` lifecycleと明示許可リスト型付きRPCです。プロファイル、権限、パッケージングは未実装です。
