...
次はWSLナビゲーション制御スパイク、続いてM3の`nimino-core` lifecycleと明示許可リスト型付きRPCです。プロファイル、権限、パッケージングは未実装です。
