import std/[algorithm, base64, httpclient, json, os, sequtils, strutils, uri]
...
proc runPackageMacos() =
  if paramCount() < 3:
    packageMacosUsage()
  ## Match Pake's macOS builder: a distributable DMG is the default while an
  ## explicit `--format app` requests a bare application bundle.
  var options = MacosPackageOptions(bundleDirectory: paramStr(2), architecture: "arm64",
    format: macosDmgPackage)
  var index = 3
  while index <= paramCount():
...
proc hostRuntimeLibraries(hostPath: string): seq[string] =
  ...
  ## then dies before it can open a window.  Take the intersection of two
  ## independent facts: the file sits beside the host that was handed to us,
  ## and the host names it.  System libraries fail the first test because they
  ## live in the Windows directory; unrelated files beside the host fail the
  ## second.  Nothing is guessed from a hard-coded list, so a host built with
  ## different dependencies stages whatever it actually needs.
  if hostPath.len == 0:
    return @[]
  let hostDirectory = parentDir(absolutePath(hostPath).normalizedPath())
  if hostDirectory.len == 0 or not dirExists(hostDirectory):
    return @[]
  var image = ""
  try:
    image = readFile(hostPath)
  except IOError, OSError:
    stderr.writeLine("nimino pack: unable to read host executable: " & hostPath)
    return @[]
  let referenced = image.toLowerAscii()
  for kind, path in walkDir(hostDirectory, relative = false):
    if kind != pcFile:
      continue
    let fileName = extractFilename(path)
    if not fileName.toLowerAscii().endsWith(".dll"):
      continue
    if referenced.contains(fileName.toLowerAscii()):
      result.add(path)
  result.sort()
...
if outputDirectory.len == 0:
  ...
  if hostPath.len > 0:
    ...
    setFilePermissions(directory / hostName, {fpUserExec, fpUserRead, fpUserWrite,
      fpGroupExec, fpGroupRead, fpOthersExec, fpOthersRead})
    for library in hostRuntimeLibraries(hostPath):
      let libraryName = extractFilename(library)
      if not copyGenerated(library, directory / libraryName):
        quit(1)
