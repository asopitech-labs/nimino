...
`multipleWebViews`、Windows/Linuxのnative menu、system tray、native notification、web permission eventsは実装済みです。WebView内部のcustom resource schemeはWindows WebView2の`WebResourceRequested`とLinux WebKitGTKのURI scheme callbackへ接続済みで、WSL relayは未実装のためWSLでは`unsupported`です。`transparentWindow`は未対応として明示的に`false`または`unsupported`を返します。Capabilityが`true`でも、権限または現在の状態による失敗は別途結果で返します。
