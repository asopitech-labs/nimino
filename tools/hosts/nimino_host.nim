...
proc main() =
  ...
  let popupConfigured = window.onNewWindow(proc(request: NewWindowRequest): bool =
    ...
    of popupLinkAllow:
      ...
      true
    of popupLinkNavigate:
      ## Pake parity: an internal target without `--new-window` replaces the
      ## requesting document, keeping SSO continuations inside the app.
      discard window.loadUrl(request.url)
      true
